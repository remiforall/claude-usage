...
`dashboard.py` serves a single-page dashboard on `localhost:8080` with Chart.js charts (served locally from `vendor/chart.umd.min.js`, no external CDN). It auto-refreshes every 30 seconds and supports model filtering with bookmarkable URLs. The bind address and port can be overridden with `HOST` and `PORT` environment variables (defaults: `localhost`, `8080`).
